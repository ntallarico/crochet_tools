...
def main(argv):
	...
	guivar_include_nums_in_cells = tk.IntVar()
	...
	entry_num_colors.insert(0, "5")
	entry_num_colors.pack(fill="both", expand=True)

	# Checkbox: use DMC color palette
	...
	checkbox_use_dmc_colors.pack(fill="both", expand=True)

	## Checkbox: include nums in cells
	checkbox_include_nums_in_cells = tk.Checkbutton(text="Include nums in cells", variable=guivar_include_nums_in_cells, onvalue=True, offvalue=False)
	checkbox_include_nums_in_cells.pack(fill="both", expand=True)

	## Next
	button_preview = tk.Button(window, font=font, text="Next", width=window_width
							, command=lambda : show_preview(guivar_checkbox_use_dmc.get(), entry_width.get(), entry_height.get(), entry_num_colors.get(), guivar_include_nums_in_cells.get()))
